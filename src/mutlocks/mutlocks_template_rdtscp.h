...
unsigned long long clockval;
unsigned long long avg_cslen;
unsigned long long avg_delay;
unsigned long long csnum;
unsigned long long lwnum;				/*40*/
int wuc;								/*44*/
int hcnt;								/*48*/
char dec_scale;							/*49*/
char inc_scale;							/*50*/
...
//		printf("DAJEEEEEEEE\n");
	...
	tmpclock = read_tsc_p();
	mutlock->avg_delay += ((-((long long)(slept && mutlock->clockval != 0))) & (tmpclock - mutlock->clockval));
	mutlock->lwnum += (slept && mutlock->clockval != 0);
	mutlock->clockval = tmpclock;
...
//	assert(wuc == -1 || wuc == 1 || wuc == 0 );
	...
	tmpclock = read_tsc_p();
	mutlock->avg_cslen += (tmpclock - mutlock->clockval);
	mutlock->clockval = ((thc == 1) ? (0) : (tmpclock));
	++mutlock->csnum;
...
static inline int FUNC_NAME(NAME, _mutlock_destroy)(__HLOCK_OBJ(NAME)* mutlock){
