 ...
 */
int freq_mutlock_wrap_init(freq_mutlock_wrapper_t* mutlock,  __HLOCK_PARAM_OBJ(NAME)* params) {
 ...
 */ 
int freq_mutlock_wrap_lock(freq_mutlock_wrapper_t* mutlock){
	if (!mutlock) return FAIL;
...
int freq_mutlock_wrap_unlock(freq_mutlock_wrapper_t* mutlock) {
	if (!mutlock) return FAIL;
...
int freq_mutlock_wrap_destroy(freq_mutlock_wrapper_t* mutlock) {
	if (!mutlock) return FAIL;
	return SUCCESS;	
}
