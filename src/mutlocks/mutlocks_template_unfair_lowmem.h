#ifndef _MUTLOCK_TEMPLATE_UNFAIR_LOWMEM_H_
#define _MUTLOCK_TEMPLATE_UNFAIR_LOWMEM_H_
...
typedef __attribute__((aligned(128))) struct __HLOCK_STRUCT(NAME) {
#if COND_VAR
    pthread_mutex_t posix_lock;
    char __pad[pad_to_cache_line(sizeof(pthread_mutex_t))]; //128?
#endif

volatile unsigned long long lstate;  							// 8
__SPIN_OBJ(NAME) spin_obj;									
char pad[8-sizeof(__SPIN_OBJ(NAME)	)];							// 16
int wuc;														 	
int hcnt;														// 24	
int cores;														 
int trylocked;														// 32	
__SLEEP_OBJ(NAME) sleep_obj;									
char pad3[128 - 4*sizeof(long long)-sizeof(__SLEEP_OBJ(NAME))];	// 128	
} __HLOCK_OBJ(NAME);
